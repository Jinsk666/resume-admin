	...
	<div
		...
		<el-collapse-item
			...
			<template slot="title">
				...
				<span class="step-edit right" @click.stop="editStep(stepData.index, stepData.data.moduleName)" v-show="stepData.index === stepData.mouseOverIndex">
					<i class="el-icon-edit"></i>
				</span>
			</template>
			<div v-if="stepData.data.moduleName != '种植' && stepData.data.moduleName != '加工' && stepData.globalPool[stepData.data.moduleDataCode]">
				<el-row
					class="acc-row"
			...
			<!-- 种植处理 -->
			<div v-else-if="(stepData.data.moduleName == '种植' || stepData.data.moduleName == '加工') && stepData.globalPool[stepData.data.moduleDataCode]">
				<el-tabs v-model="activeName2">
				<el-tab-pane
					:label="item.label || '无返回'"
					:name="index+'h'"
					v-for="(item, index) in stepData.globalPool[stepData.data.moduleDataCode].subModelInfoList"
					v-if="stepData.globalPool[stepData.data.moduleDataCode].subModelInfoList"
					:key="(index + 300)">
					<span slot="label">
					...
					<div class="item-container" v-if="item.label == '种植基本信息' ||  item.label == '环境信息' || item.label == '加工基本信息'">
						<div class="env-info">
